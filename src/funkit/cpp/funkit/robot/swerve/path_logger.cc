...
void PathLogger::RecordPose(
    const funkit::robot::swerve::odometry::SwervePose& pose) {
  ...
  try {
    ...
    for (const auto& record : recorded_poses_) {
      file << record.timestamp.to<double>() << ","
           << record.pose.position[0].value() << ","
           << record.pose.position[1].value() << ","
           << record.pose.bearing.value() << ","
           << record.pose.velocity[0].value() << ","
           << record.pose.velocity[1].value() << std::endl;
    }
