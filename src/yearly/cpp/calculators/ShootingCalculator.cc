...
void ShootingCalculator::Calculate(const RobotContainer* container_) {
  ...
  ShootingCalculatorInput in{
      .position = drivetrain_readings.estimated_pose.position,
      .velocity = drivetrain_readings.estimated_pose.velocity,
      .bearing = drivetrain_readings.estimated_pose.bearing,
      .yaw_rate = drivetrain_readings.yaw_rate,
      .is_blue_alliance = is_blue};

  outputs_ = CalculateFromInput(in);
}

ShootingCalculatorOutputs ShootingCalculator::CalculateFromInput(
  ...
  const Vector2D red_target{158.845_in_, 182.11_in_};
  Vector2D target;
  if (in.is_blue_alliance) {
    target = {funkit::math::FieldPoint::field_size_x - red_target[0],
        funkit::math::FieldPoint::field_size_y - red_target[1]};
  } else {
    target = red_target;
  }

  ShootingCalculatorOutputs out{0_deg_, 0_radps_, 0_fps_, false};
  ...
  */
  ...
  const Vector2D robot_to_shooter =
      Vector2D{0_in_, -6_in_}.rotate(in.bearing, true);
  const Vector2D shooter_pos = in.position + robot_to_shooter;

  const auto tangential_speed = robot_to_shooter.magnitude() *
                                pdcsu::units::radps_t{in.yaw_rate} / 1_rad_;
  const Vel2D vel_at_shooter =
      in.velocity + Vel2D{tangential_speed,
                        robot_to_shooter.angle(true) + 90_deg_,
  ...
  */
  ...
  degree_t aim_angle = in.position.angleAimTowards(
