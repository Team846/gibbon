...
AutoIntakeCommand::AutoIntakeCommand(RobotContainer &container, HoptakeState target)
    : funkit::robot::GenericCommand<RobotContainer, AutoIntakeCommand>{
          container, "auto_intake_command"}, target_{target}  {
...
void AutoIntakeCommand::Periodic() {
    ...
    target.drivetrain_vel = container_.drivetrain_.GetReadings().estimated_pose.velocity.magnitude();
