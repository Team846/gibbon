#include "commands/general/auto_track_command.h"

#include "calculators/ShootingCalculator.h"
#include "funkit/math/fieldpoints.h"

AutoTrackCommand::AutoTrackCommand(
    RobotContainer &container, TrackingState target)
    : funkit::robot::GenericCommand<RobotContainer, AutoTrackCommand>{container,
          "auto_track_command"},
      target_{target} {
  AddRequirements({&container_.scorer_ss_});
}

void AutoTrackCommand::OnInit() {}

void AutoTrackCommand::Periodic() {
  ...
  scorer_target.shoot = false;
  scorer_target.tracking_state = target_;

  container_.scorer_ss_.SetTarget(scorer_target);
}

void AutoTrackCommand::OnEnd(bool interrupted) {}

bool AutoTrackCommand::IsFinished() { return true; }