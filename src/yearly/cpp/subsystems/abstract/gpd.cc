...
GPDReadings GPDSubsystem::ReadFromHardware() {
  ...
  //       ms_t(latency_entry.GetLastChange());
  ...
  //   auto vel_x_ =
  //       inch_t{drivetrain_readings.estimated_pose.velocity[0].value()
  //       * 12.0};
  //   auto vel_y_ =
  //       inch_t{drivetrain_readings.estimated_pose.velocity[1].value()
  //       * 12.0};
  //   auto yaw_rate_deg_per_sec =
  //   degps_t{drivetrain_readings.yaw_rate.value()};

  auto bearing_angle =
      drivetrain_readings.estimated_pose.bearing + degree_t{theta_x} - 90_deg_;
